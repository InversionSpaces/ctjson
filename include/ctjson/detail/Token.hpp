 ...
 */
template <typename T> struct TokenValue {
    using value_type = T;

    T value;
};

template <> struct TokenValue<void> {};
 ...
 */
template <TokenType t_type, typename... Args> struct token_of_type;
...
template <TokenType t_type> struct token_of_type<t_type> {
    using type = void;
};
 ...
 */
template <typename... Tokens> struct TokenList {
    // Alias for variant
    ...
    template <typename... Others> struct cat<TokenList<Others...>> {
        using type = TokenList<Tokens..., Others...>;
    };
};

// Alias for concatenation
template <typename L, typename R> using cat = typename L::template cat<R>::type;
...
class Token {
  ...
  public:
     ...
     */
    template <Type t_type> bool is_of_type() const {
        using ExactToken = Tokens::of_type<t_type>;
     ...
     */
    template <Type t_type> auto &value() {
        using ExactToken = Tokens::of_type<t_type>;
     ...
     * @pre is_of_type<t_type> == true
     */
    template <Type t_type> const auto &value() const {
        using ExactToken = Tokens::of_type<t_type>;
        return std::get<ExactToken>(m_token).value;
    }
     ...
     */
    template <Type t_type> static std::string name() noexcept {
