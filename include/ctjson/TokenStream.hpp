...
class TokenHandler {
  public:
    ...
    bool Int64(int64_t integer) {
        return dispatch<Token::Type::Int64>(integer);
    }
    ...
    bool EndArray(unsigned size) {
        return dispatch<Token::Type::EndArray>(size);
    }
 ...
 */
template <typename InputStream, typename Derived = void> class TokenStream {
  ...
  public:
    ...
    bool is_complete() const {
        return m_reader.IterativeParseComplete() &&
               !m_handler
                    .has_token(); // We could have one more token after peek
    }
