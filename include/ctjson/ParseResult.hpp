 ...
 */
template <typename T> class ParseResultValue {
  protected:
 ...
 */
template <> class ParseResultValue<void> {};
} // namespace detail
...
class ParseResult : public detail::ParseResultBase,
    ...
    template <typename U> friend class ParseResult;
  ...
  public:
    ...
    json_error(std::string error,
        ...
        return ParseResult(ErrorType::JSON_ERROR, {.error = std::move(error),
                                                   .path = std::move(path)});
    }
    ...
    parse_error(std::string error,
                std::optional<std::string> path = std::nullopt) {
        return ParseResult(ErrorType::PARSE_ERROR, {.error = std::move(error),
                                                    .path = std::move(path)});
    }
    ...
    static ParseResult convert_error(ParseResult<U> &&other) {
        return ParseResult(other.m_error_type,
                           std::move(other.m_error.value()));
    }
    ...
    bool is_parse_error() const {
        return m_error_type == ErrorType::PARSE_ERROR;
    }
